...
import { AUTH_MODE } from './http-context-tokens';
...
export const JwtInterceptor: HttpInterceptorFn = (req, next) => {
  ...
  switch (mode) {
    ...
    case 'sid': {
      const sidReq = req.clone({
        // headers: req.headers.set('sid', environment.baseToken)
        headers: req.headers.set('Authorization', 'sid '+ environment.baseToken)
      });
      return next(sidReq);
    }
    ...
    default: {
      ...
      const authReq = req.clone({
        headers: req.headers.set('Authorization', `tma ${token}`)
      });
      return next(authReq);
    }
  }
